...
import { CodeXml, PlusIcon, XIcon } from 'lucide-react'
...
export default function OtherFields() {
	...
	const parseFormData = (): OtherFieldsType => {
		...
		return {
          ...
          awardUri: formData.get(
						`fundingReferences[${i}].awardUri`,
					) as string,
	...
	const handleFinish = () => {
		if (validateAndSave('next')) navigate('/')
	}

	return (
		<div className="mx-auto flex flex-col gap-8 p-4 lg:flex-row">
			<form
				ref={formRef}
			...
			>
				...
				<section>
						...
						return (
							<div key={index} className="mb-4 flex items-start gap-2">
								...
								<Button
									...
									size="icon"
									onClick={() => {
										setAlternateIdCount(prev => prev - 1)
					...
					<Button
						variant="secondary"
						onClick={() => setAlternateIdCount(prev => prev + 1)}
					>
				...
				<section>
					...
					<Button
						variant="secondary"
						onClick={() => setSizeCount(prev => prev + 1)}
					>
				...
				<section>
					...
					<Button
						variant="secondary"
						onClick={() => setFormatCount(prev => prev + 1)}
					>
				...
				<section>
					...
					<Button
						variant="secondary"
						onClick={() => setRightsCount(prev => prev + 1)}
					>
                ...
                <Input
								...
								{index > 0 && (
									<Button
										...
										onClick={() => {
											setFundingRefCount(prev => prev - 1)
											// Clear errors for removed item
											setErrors(prev => {
												const newErrors = { ...prev }
												Object.keys(prev).forEach(key => {
					...
					<Button
						variant="secondary"
						onClick={() => setFundingRefCount(prev => prev + 1)}
					>
				...
				<div className="mt-8 flex gap-4">
					<Button onClick={handleFinish}>Finish & Go to Home</Button>
					<Button variant="outline" onClick={handlePreview} type="button">
						<CodeXml className="mr-2 h-4 w-4" /> Preview XML
					</Button>
				</div>
			...
			{/* Desktop XML preview */}
			<div className="sticky top-0 hidden h-fit max-h-[calc(100vh-5rem)] flex-1 overflow-auto lg:block">
				<div className="mb-4">
					<h2 className="mb-2 text-xl font-semibold">XML Preview</h2>
					<Button variant="outline" onClick={handlePreview} className="mb-4">
						<CodeXml className="mr-2 h-4 w-4" /> Generate Preview
					</Button>
				</div>
				<XmlOutput xmlOutput={xmlOutput} />
			</div>
